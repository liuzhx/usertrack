         ...
         */
			...
			/* remove ipv6
                           if(strcmp(op->OidName,CISCO_IETF_IP_MIB_cInetNetToMediaPhysAddress) == 0 && (*pL3)->SwitchType == 3){
                            ...
                            }else if(strcmp(op->OidName,IF_MIB_ifName) == 0 ){
                                if((*pL3)->SwitchType == 3){
				/*remove ipv6
				...
				*/
					 ...
					 if (strncmp("Vlan",vars->val.string,4)==0){
					    if ( Sw3Vlan_v4_TBL[routerindex][i].vlanid == vars->name[11] ){
						 strncpy(Vlan_buf,vars->val.string+14,3);
						 Sw3Vlan_v4_TBL[routerindex][i].vlanid = atoi (Vlan_buf);  
